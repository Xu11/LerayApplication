...
dt = 0.01
comm = MPI.comm_world
file_handler_press = XDMFFile('output/press_semiImplicit_BDF2.xdmf')
file_handler_vel   = XDMFFile('output/vel_semiImplicit_BDF2.xdmf')
file_handler_lift = XDMFFile('output/lift_semiImplicit_BDF2.xdmf')
file_handler_drag = XDMFFile('output/drag_semiImplicit_BDF2.xdmf')
file_handler_pressureDiff = XDMFFile('output/pressureDiff_semiImplicit_BDF2.xdmf')
...
## Read XDMF mesh-------------------------------------------------------
meshPath="output/mesh.xdmf"
boundaryMeshPath="output/facet.xdmf"
...
def build_nullspace(V):
    ...
    V.sub(1).dofmap().set(nullspace_basis[0], 1.0)
    # Define Null Space or Dirichlet Datum.
    for x in nullspace_basis:
        x.apply("insert") ##finalize the assembly of the vector?
...
for i in range(801):
    ...
    D = (-1*force[0]*20)*(ds(6)+ds(7)+ds(8)+ds(9))
...
if __name__ == "__main__":
   ...
   # print(lift_array)
   np.savetxt('lift_simu.csv',lift_array,fmt='%1.3f,%1.6e',header="time,lift_simu",comments='')
   np.savetxt('drag_simu.csv',drag_array,fmt='%1.3f,%1.6e',header="time,drag_simu",comments='')
   np.savetxt('pressDiff_simu.csv',pDiff_array,fmt='%1.3f,%1.6e',header="time,pressDiff_simu",comments='')
