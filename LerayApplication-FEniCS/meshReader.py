# Author T. Landet
import dolfin
def load_meshio_mesh(mesh, file_name, meshio_file_type=None, sort_order=None,dim_input=2):
    """
    ...
    * xdmf
    ...
    if sort_order is not None and len(connectivity)>0:
...
def readMesh(file_name,file_type="gmsh",sort_order=None):
    pfile="output/facet.xdmf"
    mfile="output/mesh.xdmf"
    #file_type="gmsh"
    #sort_order=[2,0,1]
    ...
    #Facet Vertex relation
    conn_FV =  mesh.topology()(mesh.topology().dim() - 1, mesh.topology().dim() - 2)
    ...
    with dolfin.XDMFFile(comm, mfile) as xdmf:
        xdmf.write(mesh)
    #import pdb
    #pdb.set_trace()
    return mesh,mfr
